...
class TextCompare{
    ...
    *******************************************************************************/
    private static function isolate($srcTxt, $symbol, $numofsymbols){
        ...
        $y = 1;
        do {
            $srcTxt = TextCompare::strafter($srcTxt,$symbol);
            $z = ($y-1);
            $strarray[$y] = $srcTxt;
    ...
    *******************************************************************************/
    public static function processText($scrtxt){
        $text = TextCompare::formatText($scrtxt, 'x_#_z');
        $num  = TextCompare::sentences_num($text,'x_#_z');
        $txt = TextCompare::strafter($text,'[#_start]');
        $arr = TextCompare::isolate($txt, 'x_#_z', $num);
        return $arr;
    }
    ...
    *******************************************************************************/
    ...
    public static function compareText($scrtxt_1, $scrtxt_2, $level = 60, $specialchars = false ){
        ...
        // $arr_1 = TextCompare::processText($scrtxt_1);
        // $arr_2 = TextCompare::processText($scrtxt_2);

        $arr_1 = TextCompare::nGrams($scrtxt_1);
        $arr_2 = TextCompare::nGrams($scrtxt_2);

        foreach ($arr_1 as $key_1 => $value_1){
            $nr1 = $key_1;
            if ($specialchars==true){
                $val_1 = TextCompare::cleanText($value_1);
            }else{
                $val_1 = $value_1;
            }
            foreach ($arr_2 as $key_2 => $value_2){
                $nr2 = $key_2;
                if ($specialchars==true){
                    $val_2 = TextCompare::cleanText($value_2);
                } else{
                    $val_2 = $value_2;
                }
                $index = TextCompare::correlationText($val_1,$val_2);
                if ($index >= $level){
